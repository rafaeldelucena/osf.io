/**
 * Module that controls the GitLab node settings. Includes Knockout view-model
 * for syncing data.
 */
...
function ViewModel(url) {
    ...
    self.gitlabs = ko.observableArray([]);
    self.datasets = ko.observableArray([]);

    self.savedDatasetDoi = ko.observable();
    self.savedDatasetTitle = ko.observable();
    self.savedGitLabAlias = ko.observable();
    self.savedGitLabTitle = ko.observable();
    ...
    self.messages = {
        ...
        authError: ko.pureComputed(function() {
            return 'Sorry, but there was a problem connecting to that instance of GitLab. It ' +
                'is likely that the instance hasn\'t been upgraded to GitLab 4.0. If you ' +
        ...
        datasetDeaccessioned: ko.pureComputed(function() {
            return 'This dataset has already been deaccessioned on GitLab ' +
    ...
    self.savedGitLabUrl = ko.pureComputed(function() {
        return (self.urls()) ? self.urls().gitlabPrefix + self.savedGitLabAlias() : null;
    });

    self.selectedGitLabAlias = ko.observable();
    self.selectedDatasetDoi = ko.observable();
    self.selectedGitLabTitle = ko.pureComputed(function() {
        for (var i = 0; i < self.gitlabs().length; i++) {
            var data = self.gitlabs()[i];
            if (data.alias === self.selectedGitLabAlias()) {
                return data.title;
            }
        }
        return null;
    ...
    self.gitlabHasDatasets = ko.pureComputed(function() {
        return self.datasets().length > 0;
    });

    self.showDatasetSelect = ko.pureComputed(function() {
        return self.loadedDatasets() && self.gitlabHasDatasets();
    });
    ...
    self.showLinkGitLab = ko.pureComputed(function() {
    ...
    self.hasGitLabs = ko.pureComputed(function() {
        return self.gitlabs().length > 0;
    });
    ...
    self.enableSubmitDataset = ko.pureComputed(function() {
        return !self.submitting() && self.gitlabHasDatasets() &&
...
ViewModel.prototype.updateFromData = function(data) {
    ...
    if (self.nodeHasAuth()) {
        self.savedHost(data.gitlabHost);
        self.gitlabs(data.gitlabs);
        self.savedGitLabAlias(data.savedGitLab.alias);
        self.savedGitLabTitle(data.savedGitLab.title);
        self.selectedGitLabAlias(data.savedGitLab.alias);
...
ViewModel.prototype.setInfo = function() {
    ...
    return $osf.postJSON(
        ...
        ko.toJS({
            gitlab: {
                alias: self.selectedGitLabAlias
            },
            dataset: {
        ...
        self.savedGitLabAlias(self.selectedGitLabAlias());
        self.savedGitLabTitle(self.selectedGitLabTitle());
        self.savedDatasetDoi(self.selectedDatasetDoi());
...
ViewModel.prototype.getDatasets = function() {
    ...
    return $osf.postJSON(
        ...
        // Don't update if another GitLab has been selected
        if (response.alias === self.selectedGitLabAlias()) {
...
function GitLabNodeConfig(selector, url) {
    // Initialization code
    var self = this;
    ...
    $osf.applyBindings(self.viewModel, '#gitlabScope');
}
module.exports = GitLabNodeConfig;
