...
var _gitlabItemButtons = {
    view: function (ctrl, args, children) {
        ...
        function gitlabPublish(event, item, col) {
            var both = !item.data.gitlabIsPublished;
            var url = item.data.urls.publish;
            var host = item.data.host;
            var toPublish = both ? 'GitLab and dataset' : 'dataset';
            // Set the modal content to reflect the file's external host
            var modalContent = [
                m('p.m-md', both ? 'This dataset cannot be published until the ' + item.data.gitlab + ' GitLab is published. ' : ''),
                ...
                m('p.font-thick.m-md', both ? 'Do you want to publish this GitLab AND this dataset?' : 'Are you sure you want to publish this dataset?')
            ];
            var modalActions =
            ...
            function publishDataset() {
                ...
                $.osf.putJSON(
                    ...
                    item.data.gitlabIsPublished = true;
                    ...
                    switch (statusCode) {
                    case 405:
                        message = 'Error: This dataset cannot be published until ' + item.data.gitlab + ' GitLab is published.';
                        break;
                    case 409:
            ...
            if (item.kind === 'folder' && item.data.addonFullname && item.data.version === 'latest' && item.data.permissions.edit) {
                ...
                if(item.data.datasetDraftModified) {
                    buttons.push(
                        m.component(Fangorn.Components.button, {
                            onclick: function (event) {
                                gitlabPublish.call(tb, event, item);
                            },
                            icon: 'fa fa-globe',
...
function _fangornGitLabTitle(item, col) {
    ...
    if (item.data.addonFullname) {
        var contents = [m('gitlab-name', item.data.name + ' (' + version + ')')];
